...
1. **Laspeyres**: uses base-period quantities as weights.

2. **Paasche**: uses current-period quantities as weights.

These are used to measure the evolution of *prices* or *quantities*.
...
## Kids' Explanation - Quantities

Laspeyres - Quantity: Use 🏷️$_{past}$! Laspeyres Quantity Index (LQI): $LQI_{old}=10\times 1 + 5 \times 0.8 = 14$, and $IPL_{new}=8\times 1 + 6\times 0.8=`12.8`$.
If the base year is *old* then the index would be $100$ in *old* and `r round((12.8/14)*100, 1)` for the year *new*.

\

Paasche - Quantity: Use 🏷️$_{today}$! Paasche Quantity Index (PQI): $PQI_{old}=10\times 1.2 + 6 \times 1.1 = 18.6$, and $IPL_{new}=8\times 1.2 + 6\times 1.1=16.2$.
If the base year is *old* then the index would be $100$ in *old* and `r round((16.2/18.6)*100, 1)` for the year *new*. :label:
