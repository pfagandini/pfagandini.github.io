...
\begin{tikzpicture}[
  ...
  every node/.style={draw, rounded corners, align=center, minimum height=1.2cm, minimum width=2.5cm},
  ->, thick
]

\node (A) {Index Number};
\node (B) [right=of A, yshift=1.2cm] {Prices $P$};
\node (C) [right=of A] {Quantities $Q$};
\node (D) [right=of A, yshift=-1.2cm] {Value $P \times Q$};
