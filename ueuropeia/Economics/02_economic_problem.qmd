...
# Define the function: f(x) = 5 - 0.25x
f <- function(x) 5 - 0.25 * x

# Data for the line on [0, 20] only
line_df <- data.frame(
  x = seq(0, 20, length.out = 200)
)
line_df$y <- f(line_df$x)

# Points at x = 0, 4, 8, 12, 16, 20 with labels A-F
x_pts <- seq(0, 20, by = 4)
pts_df <- data.frame(
  x = x_pts,
  y = f(x_pts),
...
ggplot() +
  geom_line(data = line_df, aes(x, y), linewidth = 1.2, color = "#2b8cbe") +
  geom_point(data = pts_df, aes(x, y), size = 2.8, color = "#2b8cbe") +
  geom_text(data = pts_df, aes(x, y, label = lab),
            nudge_x = 0.4, nudge_y = 0.15, size = 4, color = "#333333") +
  annotate("text", x = 2, y = 5.5, label = "f(x) = 5 - 0.25x", hjust = 0,
           size = 4.2, color = "#444") +
  scale_x_continuous(limits = c(0, 24)) +
  scale_y_continuous(limits = c(0, 6)) +
  labs(title = "Budget constraint for Alphonso, with a budget of 20",
       x = "Bus tickets",
       y = "Burgers") +
  ...
  theme(
   ...
   annotate("segment", x = 0, xend = 24, y = 0, yend = 0,
           arrow = arrow(length = unit(0.3, "cm"), type = "closed")) +
  annotate("segment", x = 0, xend = 0, y = 0, yend = 6,
           arrow = arrow(length = unit(0.3, "cm"), type = "closed")) +
  annotate("text", x = 24, y = -0.2, label = "x", hjust = 1, vjust = 1, size = 5) +
  annotate("text", x = -0.5, y = 6, label = "y", hjust = 1, vjust = 1, size = 5)
